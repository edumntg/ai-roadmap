...
def make_fooling_image(X, target_y, model):
    ...
    predicted_label = -1
    while predicted_label != target_y:
        # Predict
        ...
        # Update Fooling image
        X_fooling.data += learning_rate * X_fooling.grad / X_fooling.grad.norm()
        X_fooling.grad.zero_() # set gradients to zero

        predicted_label = y_pred.max(dim=1)[1]
...
def class_visualization_update_step(img, model, target_y, l2_reg, learning_rate):
    ...
    # Compute loss and gradients
    loss = y_pred[0, target_y] - l2_reg * img.square().sum()
    loss.backward()
