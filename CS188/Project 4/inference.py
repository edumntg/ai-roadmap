...
def constructBayesNet(gameState: hunters.GameState):
    ...
    # Now, for observations, include all possible distances with all possible noises
    seen = set()
    for (x1, y1) in variableDomainsDict[PAC]:
        for (x2, y2) in variableDomainsDict[PAC]:
            for noise in range(MAX_NOISE+1):
                distance = manhattanDistance((x1,y1), (x2,y2)) + noise
                if not distance in seen:
                    variableDomainsDict[OBS0].append(distance)
                    variableDomainsDict[OBS1].append(distance)
                    seen.add(distance)
